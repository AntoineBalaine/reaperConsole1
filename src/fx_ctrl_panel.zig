...
pub fn drawFxControlPanel(ctx: imgui.ContextPtr, state: *State) !?UpdateEvent {
    ...
    // Input Section
    {
        ...
        if (try Knobs.drawWidget(ctx, &gain_prm.normalized, gain_prm.label, .{})) {
            return .{ .cc = .Inpt_Gain, .value = gain_prm.normalized };
        }
    ...
    // Shape Section
    {
        ...
        if (try imgui.Checkbox(.{ ctx, "SHAPE##shp", &shp_bypass_val })) {
            return .{ .cc = .Shp_shape, .value = if (shp_bypass_val) 1 else 0 };
        }
        ...
        // First column
        {
            ...
            defer imgui.EndGroup(.{ctx}) catch {};
            {
                ...
                if (try Knobs.drawWidget(ctx, &gate_prm.normalized, gate_prm.label, .{})) {
                    return .{ .cc = .Shp_Gate, .value = gate_prm.normalized };
                }
                ...
                if (try imgui.Checkbox(.{ ctx, "hard gate##hardgate", &hard_gate_val })) {
                    return .{ .cc = .Shp_hard_gate, .value = if (hard_gate_val) 1 else 0 };
                }
            }
        }
        ...
        // Second column
        {
            ...
            defer imgui.EndGroup(.{ctx}) catch {};
            {
                ...
                if (try Knobs.drawWidget(ctx, &release_prm.normalized, release_prm.label, .{})) {
                    return .{ .cc = .Shp_GateRelease, .value = release_prm.normalized };
                }
    ...
    // EQ Section
    {
        ...
        if (try imgui.Checkbox(.{ ctx, "EQ##eq", &eq_bypass_val })) {
            return .{ .cc = .Eq_eq, .value = if (eq_bypass_val) 1 else 0 };
        }
        ...
        { // second col
            ...
            defer imgui.EndGroup(.{ctx}) catch {};
            {
                ...
                if (try Knobs.drawWidget(ctx, &lomidq_prm.normalized, lomidq_prm.label, .{})) {
                    return .{ .cc = .Eq_LoMidQ, .value = lomidq_prm.normalized };
                }
                ...
                if (try Knobs.drawWidget(ctx, &lomidfreq_prm.normalized, lomidfreq_prm.label, .{})) {
                    return .{ .cc = .Eq_LoMidFrq, .value = lomidfreq_prm.normalized };
                }
                ...
                if (try Knobs.drawWidget(ctx, &lomidgain_prm.normalized, lomidgain_prm.label, .{})) {
                    return .{ .cc = .Eq_LoMidGain, .value = lomidgain_prm.normalized };
                }
            }
        }
        ...
        { // third col
            ...
            defer imgui.EndGroup(.{ctx}) catch {};
            {
                ...
                if (try Knobs.drawWidget(ctx, &himidq_prm.normalized, himidq_prm.label, .{})) {
                    return .{ .cc = .Eq_HiMidQ, .value = himidq_prm.normalized };
                }
                ...
                if (try Knobs.drawWidget(ctx, &himidfreq_prm.normalized, himidfreq_prm.label, .{})) {
                    return .{ .cc = .Eq_HiMidFrq, .value = himidfreq_prm.normalized };
                }
                ...
                if (try Knobs.drawWidget(ctx, &himidgain_prm.normalized, himidgain_prm.label, .{})) {
                    return .{ .cc = .Eq_HiMidGain, .value = himidgain_prm.normalized };
                }
            }
        }
    ...
    // Compressor Section
    {
        ...
        if (try imgui.Checkbox(.{ ctx, "Compressor##comp_byp", &comp_bypass_val })) {
            return .{ .cc = .Comp_comp, .value = if (comp_bypass_val) 1 else 0 };
        }
        ...
        // First column
        {
            ...
            defer imgui.EndGroup(.{ctx}) catch {};
            {
                ...
                if (try Knobs.drawWidget(ctx, &ratio_prm.normalized, ratio_prm.label, .{})) {
                    return .{ .cc = .Comp_Ratio, .value = ratio_prm.normalized };
                }
                ...
                if (try Knobs.drawWidget(ctx, &drywet_prm.normalized, drywet_prm.label, .{})) {
                    return .{ .cc = .Comp_DryWet, .value = drywet_prm.normalized };
                }
            }
        }
        ...
        // Second column
        {
            ...
            defer imgui.EndGroup(.{ctx}) catch {};
            {
                ...
                if (try Knobs.drawWidget(ctx, &attack_prm.normalized, attack_prm.label, .{})) {
                    return .{ .cc = .Comp_Attack, .value = attack_prm.normalized };
                }

                var release_prm = &fx_state.values.getPtr(.Comp_Release).?.param;
                if (try Knobs.drawWidget(ctx, &release_prm.normalized, release_prm.label, .{})) {
                    return .{ .cc = .Comp_Release, .value = release_prm.normalized };
                }
                ...
                if (try Knobs.drawWidget(ctx, &thresh_prm.normalized, thresh_prm.label, .{})) {
                    return .{ .cc = .Comp_Thresh, .value = thresh_prm.normalized };
                }
            }
        }
    }
        ...
        // Second column
        {
            ...
            defer imgui.EndGroup(.{ctx}) catch {};
            {
                ...
                if (try imgui.Checkbox(.{ ctx, "solo##out_solo", &solo_val })) {
                    return .{ .cc = .Out_solo, .value = if (solo_val) 1 else 0 };
                }
